...
const enrichRecordQuery = (query) =>
  query.populate("assignedTo", responderProjection).populate("userId", responderProjection);

const sanitizeFileName = (name = "attachment") =>
  name.replace(/[^a-zA-Z0-9._-]/g, "_").slice(0, 100);

const getIncidentAuthQuery = (user, id) =>
  canManageAllIncidents(user.role) ? { _id: id } : { _id: id, userId: user.id };

const ensureIncidentAccess = async (user, id) => {
  return await Record.findOne(getIncidentAuthQuery(user, id)).select("_id");
};

const getIncidentAttachmentDir = (incidentId) =>
  path.join(uploadsRoot, incidentId);
...
export const createRecord = async (req, res, next) => {
  try {
    const { title, content, description, metadata, severity, assignedTo, tags } =
      req.body;
    ...
    const hydratedRecord = await enrichRecordQuery(
      Record.findById(record._id)
    );

    broadcast("incident_created", hydratedRecord);

    logger.trackEvent("record_created", {
      userId: req.user.id,
...
export const getRecordById = async (req, res, next) => {
  try {
    const incident = await enrichRecordQuery(
      Record.findOne(getIncidentAuthQuery(req.user, req.params.id))
    );
...
export const updateRecord = async (req, res, next) => {
  try {
    ...
    const { title, content, description, metadata, severity, status, assignedTo } =
      req.body;
    ...
    const record = await enrichRecordQuery(
      Record.findOneAndUpdate(query, update, { new: true })
    );
    ...
    logger.trackEvent("record_updated", {
      userId: req.user.id,
      recordId: record._id.toString(),
    });
...
export const deleteRecord = async (req, res, next) => {
  try {
    ...
    logger.trackEvent("record_deleted", {
      userId: req.user.id,
      recordId: record._id.toString(),
    });
...
export const uploadAttachment = async (req, res, next) => {
  try {
    ...
    if (!fileName || !contentBase64) {
      return res
        .status(400)
        .json({ message: "fileName and contentBase64 are required" });
    }
    ...
    try {
      ...
      return res
        .status(400)
        .json({ message: "contentBase64 must be valid base64" });
    }

    if (!buffer.length) {
      return res.status(400).json({ message: "Attachment content is empty" });
    }

    if (buffer.length > 10 * 1024 * 1024) {
      return res
        .status(413)
        .json({ message: "Attachment too large (max 10MB)" });
    }
...
export const downloadAttachment = async (req, res, next) => {
  try {
    ...
    const filePath = path.join(
      getIncidentAttachmentDir(String(incident._id)),
      safeName
    );
